...
export default function TicketDetailPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 font-sans pb-28 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto pt-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <p className="text-sm text-gray-500">{complaint.employeeId}</p>
          </div>

          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <h3 className="text-sm font-medium text-gray-500 mb-1">Team ID</h3>
            <p className="text-gray-800">{complaint.teamId}</p>
          </div>
      ...
      {/* Attachments Section */}
{ticket.data.uploads && ticket.data.uploads.length > 0 && (
  <section className="max-w-4xl mx-auto mt-10">
    <h2 className="text-lg font-semibold text-gray-800 mb-4">Attachments</h2>
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      {ticket.data.uploads.map((url: string, index: number) => {
          ...
          >
            {isImage ? (
              ...
              <a
                ...
                className="block p-4 text-blue-600 hover:underline"
              >
                View File
              </a>
            )}
